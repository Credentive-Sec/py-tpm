...
from .helper_functions import intToTpm, intFromTpm
...
class TpmBuffer:
    def __init__(self, lengthOrSrcBuf: Union[TpmBuffer, bytearray, int] = 4096):
        if isinstance(lengthOrSrcBuf, TpmBuffer):
            self.__buf = bytearray(lengthOrSrcBuf.buffer)
    ...
    # def init(self):
        self.curPos = 0
        self.__length: int = len(self.buffer)
        self.__sizedStructSizes: list[SizedStructInfo] = []
    ...
    def writeNumAtPos(self, val, pos, len: int = 4):
        curPos = self.curPos
        self.curPos = pos
