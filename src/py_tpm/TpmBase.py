...
class Session:
    def __init__(
        ...
        nonceTpm=None,
        sessionAttributes: TPMA_SESSION = TPMA_SESSION.continueSession,
        nonceCaller=None,
    ):
    ...
    def Pw(authValue=None):  # Session
        s = Session(
            sessionHandle=TPM_HANDLE(TPM_RH.RS_PW),
        )
        s.SessIn.nonce = None
...
class TpmBase:
    ...
    def close(self):
        if self.__device:
            self.__device.close()
            # self.__device = None # TODO - figure out what to do with this.
    ...
    def dispatchCommand(
        ...
        if numAuthHandles > 0:
            if not self.__sessions:
                ...
                self.sessions = self.__sessions + [None] * (
                    numAuthHandles - len(self.__sessions)
                )
        ...
        cmdBuf.trim()
        respBuf: bytearray = bytearray()
        rc = TPM_RC.RETRY
        while rc == TPM_RC.RETRY:
            respBuf = bytearray(self.__device.dispatchCommand(cmdBuf.buffer))
            rc = TPM_RC(intFromTpm(buf=respBuf, pos=6, valLen=4))
    ...
    def processResponse(self, respBuf, RespType=None):
        ...
        if respBuf.curPos != 0:
            raise (
                Exception(
                    "Response buffer reading position is not properly initialized"
                )
            )
        ...
        if (
            rc == TPM_RC.SUCCESS
            and tag != int(self.__cmdTag)
            or rc != TPM_RC.SUCCESS
            and tag != int(TPM_ST.NO_SESSIONS)
        ):
            self.__generateError(
                TPM_RC.TSS_RESP_BUF_INVALID_SESSION_TAG,
                "Invalid session tag in the response buffer",
                self.__errorsAllowed,
            )
            return None
        ...
        respParamsSize = (
            ...
            else respBuf.size - respBuf.curPos
        )
        ...
        if respParamsSize != respBuf.curPos - paramStart:
            return self.__generateError(
                TPM_RC.TSS_RESP_BUF_INVALID_SIZE,
                "Inconsistent TPM response params size: expected ${exp}, actual ${act}".format(
                    exp=respParamsSize, act=respBuf.curPos - paramStart
                ),
                errorsAllowed,
            )
        return resp
